...
def joined_rows(config):
    ...
    for value in values:
        ...
        rows.append({**value, **prefixes[state_id]})
    return rows
...
def main():
    ...
    pipeline = fit_feature_pipeline(train_rows, int(training["pca_components"]))
    ...
    output = Path(config["output"]["critic_dir"]); output.mkdir(parents=True, exist_ok=True)
    ...
    report = {"train_suites": sorted(train_suites), "validation_suites": sorted(val_suites),
